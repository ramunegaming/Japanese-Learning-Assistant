...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedFavorites) {
        try {
            ...
            favorites = parsedFavorites.map(fav => {
                if (typeof fav === 'string') {
                    return { word: fav, reading: '', meaning: '' };
                }
                return fav;
            });
            updateFavoritesDisplay();
    ...
    // Add example search button event listener
    document.getElementById('example-search-button').addEventListener('click', () => {
        const query = document.getElementById('example-search-input').value.trim();
        if (query) {
            handleExampleSearch(query);
        }
    });
